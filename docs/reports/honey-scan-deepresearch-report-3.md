...
**Ergebnis:** Go kann durch Goroutines tausende Verbindungen halten. PHP muss für jeden Request den gesamten Kontext laden (Bootstrapping), was bei Flood-Angriffen zum Kollaps führt.

---

## 4. Soll-Architektur: Hybrider Ansatz

Um den Wunsch nach einfacherer Anpassbarkeit (PHP/Web) mit der notwendigen Performance (Go) zu vereinen, wird eine strikte Trennung eingeführt.

### 4.1 System-Architektur Diagramm (C4 Level 2)
...
Hier zoomen wir in die spezifischen Abläufe hinein, die während des Refactorings implementiert werden müssen.

### 5.1 Prozess: De-Localization & Schema Migration (Phase 1 & 3)

Wie wandeln wir die chinesischen Daten in englische Strukturen um?
...
### 5.2 Prozess: Datenfluss vom Sensor zum Dashboard (Detail)

Dieser Flow zeigt, wie Daten künftig fließen, ohne dass das Dashboard (PHP) den Sensor (Go) blockiert.
...
Der Zeitplan für die Umsetzung der oben gezeigten Architektur.
